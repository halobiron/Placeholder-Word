...
# Agent Teams - AK-Native Orchestration Engine
...
**Templates:** `ak:research`, `ak:cook`, `ak:code-review`, `ak:debug`
...
4. Do NOT fall back to subagents. `/ak:team` MUST use Agent Teams or abort.
...
```
AK Context:
- Work dir: {AK_PROJECT_ROOT or CWD}
- Reports: {AK_REPORTS_PATH or "plans/reports/"}
- Plans: {AK_PLANS_PATH or "plans/"}
- Branch: {AK_GIT_BRANCH or current branch}
- Naming: {AK_NAME_PATTERN or "YYMMDD-HHMM"}
- Active plan: {AK_ACTIVE_PLAN or "none"}
...
*Wraps /ak:research skill — scope, gather, analyze, report.*
...
3. **CALL** `TaskCreate` x N — one per angle:
   ...
   - Description: `Investigate <angle> for topic: <topic>. Save report to: {AK_REPORTS_PATH}/researcher-{N}-{AK_NAME_PATTERN}-{topic-slug}.md. Format: Executive summary, key findings, evidence, recommendations. Mark task completed when done. Send findings summary to lead.`

4. **CALL** `Task` x N to spawn researcher teammates:
   - `subagent_type: "researcher"`, `team_name: "<topic-slug>"`, `model: "haiku"`
   - `name: "researcher-{N}"`
   - Prompt: task description + AK Context Block
...
6. **READ** all researcher reports from `{AK_REPORTS_PATH}/`

7. **SYNTHESIZE** into: `{AK_REPORTS_PATH}/research-summary-{AK_NAME_PATTERN}-{topic-slug}.md`
...
*Wraps /ak:cook skill — plan, code, test, review, finalize.*
...
6. **DOCS SYNC EVAL** (MANDATORY for cook — from /ak:cook finalize):
   ```
...
*Wraps /ak:code-review skill — scout, review, synthesize with evidence gates.*
...
3. **CALL** `TaskCreate` x N — one per focus:
   ...
   - Description: `Review <scope> for <focus>. Output severity-rated findings only. Format: [CRITICAL|IMPORTANT|MODERATE] <finding> — <evidence> — <recommendation>. No "seems" or "probably" — concrete evidence only. Save to: {AK_REPORTS_PATH}/reviewer-{N}-{AK_NAME_PATTERN}-{scope-slug}.md. Mark task completed when done.`
...
6. **SYNTHESIZE** into: `{AK_REPORTS_PATH}/review-{scope-slug}.md`
...
*Wraps /ak:fix skill — root-cause-first, adversarial hypotheses, disprove to converge.*
...
7. **WRITE** root cause report: `{AK_REPORTS_PATH}/debug-{issue-slug}.md`
