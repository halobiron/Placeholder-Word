...
Activate `ak:debug` skill. Use `debugger` subagent if needed.
...
|------|------------------|
| 1 | `ak:debug`, `debugger` subagent |
| 2 | Multiple `Explore` subagents in parallel (optional) |
| 3 | `ak:problem-solving`, `ak:sequential-thinking`, parallel `Bash` for verification |
| 4 | `tester` subagent |
