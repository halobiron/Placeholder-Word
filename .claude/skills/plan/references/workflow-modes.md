...
4. **Context reminder:** `/ak:cook --auto {absolute-plan-path}/plan.md`
...
```
Skill(skill: "ak:plan:red-team", args: "{plan-directory-path}")
```
...
| `auto` | Use the `Skill` tool: `Skill(skill: "ak:plan:validate", args: "{plan-directory-path}")` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Use the `Skill` tool to invoke `plan:validate`:
```
Skill(skill: "ak:plan:validate", args: "{plan-directory-path}")
```
...
| Mode | Cook Command |
|------|-----------------------------|
| fast | `/ak:cook --auto {path}/plan.md` |
