...
1. **Screenshots provided** → Use `ak:ai-multimodal` skill to analyze and describe issues in detail
2. **Videos provided** → Use `ak:ai-multimodal` video-analysis to extract relevant copy issues
3. **Scout codebase** → `/ak:scout ext` (preferred) or `/ak:scout` to find files needing enhancement
