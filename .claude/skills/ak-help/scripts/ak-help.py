#!/usr/bin/env python3
"""
AgentKit Help - All-in-one guide with dynamic skill discovery.
Scans .claude/skills/ directory to build catalog at runtime.

Usage:
    python ak-help.py                    # Overview with quick start
    python ak-help.py fix                # Category guide with workflow
    python ak-help.py plan validate      # Subcommand details
    python ak-help.py debug login error  # Task recommendations
    python ak-help.py auth               # Search (unknown word)
"""
...
def emit_output_type(output_type: str) -> None:
    ...
    print(f"@AK_OUTPUT_TYPE:{output_type}")
    print()
...
TASK_MAPPINGS = {
    ...
    "config": ["config", "configure", "settings", "ak.json", ".ak.json", "setup", "locale", "language", "paths"],
...
CATEGORY_GUIDES = {
    ...
    "coding-level": {
        ...
        "tip": "Set in .ak.json. Guidelines auto-inject on session start",
    },
    "config": {
        "title": "AgentKit Configuration (.ak.json)",
        "workflow": [
            ("Global", "Set user prefs in `~/.claude/.ak.json`"),
            ("Local", "Override per-project in `./.claude/.ak.json`"),
...
SKILL_CATEGORY_MAP = {
    # AK core skills → their matching category
    "code-review": "review",
    "debug": "fix",
    "scout": "core",
    "use-mcp": "core",
...
def discover_skills(skills_dir: Path) -> dict:
    ...
    for skill_md in sorted(skills_dir.rglob("SKILL.md")):
        ...
        # Skip template and ak-help itself
        if skill_name in ('template-skill', 'ak-help'):
            continue
        ...
        # Clean description (remove [AK] prefix, emoji indicators)
        clean_desc = re.sub(r'^\[AK\]\s*', '', description).strip()
...
def show_overview(data: dict, prefix: str) -> None:
    ...
    help_cmd = f"/{prefix}ak-help" if prefix else "/ak-help"

    print("# AgentKit Skills")
    print()
...
def show_config_guide() -> None:
    """Display comprehensive .ak.json configuration guide."""
    emit_output_type("comprehensive-docs")

    print("# AgentKit Configuration (.ak.json)")
    print()
    print("**Locations (cascading resolution):**")
    print("- Global: `~/.claude/.ak.json` (user preferences)")
    print("- Local: `./.claude/.ak.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.claude/.ak.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```")
    print()
    print("**Local override** (`./.claude/.ak.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print("```bash")
    print("# ~/.claude/.ak.json - applies everywhere")
    ...
    print("# Local (.claude/.ak.json): issuePrefix = \"JIRA-\"")
...
def show_coding_level_guide() -> None:
    ...
    print("- Global: `~/.claude/.ak.json` - personal preference")
    print("- Local: `./.claude/.ak.json` - project override")
    print()
    print("---")
    print()
    print("## How It Works")
    print()
    print("1. SessionStart hook reads `codingLevel` from `.ak.json`")
...
def main():
    ...
    # .claude/skills/ak-help/scripts/ak-help.py -> .claude/skills
    ...
    if input_str.lower() in ["config", "configuration", ".ak.json", "ak.json"]:
        show_config_guide()
        return
