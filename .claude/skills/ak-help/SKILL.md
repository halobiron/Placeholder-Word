---
name: ak:ak-help
description: "AgentKit usage guide — discover commands, skills, workflows naturally."
argument-hint: "[category|command|task description]"
---

# AgentKit Help

All-in-one AgentKit guide. Run the script and present output based on type markers.
...
```bash
python .claude/skills/ak-help/scripts/ak-help.py "$ARGUMENTS"
```

## Output Type Detection

The script outputs a type marker on the first line: `@AK_OUTPUT_TYPE:<type>`

**Read this marker and adjust your presentation accordingly:**

### `@AK_OUTPUT_TYPE:comprehensive-docs`
...
### `@AK_OUTPUT_TYPE:command-details`
...
### `@AK_OUTPUT_TYPE:task-recommendations`
...
- **`/ak:plan` → `/ak:cook`**: Best for high-risk or complex changes
- **`/ak:cook`**: Standalone for straightforward implementation
- **NEVER** claim `/ak:plan` is mandatory before `/ak:cook`
