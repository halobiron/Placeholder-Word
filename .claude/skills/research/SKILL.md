...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.claude/.ak.json` (or `~/.claude/.ak.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from `.claude/.ak.json`: `gemini.model` (default: `gemini-3-flash-preview`)
