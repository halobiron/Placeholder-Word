...
Complete workflow for generating design-aligned visual assets using `ak:ai-multimodal` skill.
...
Use `ak:ai-multimodal` skill's image generation capabilities:
...
Use `ak:ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
