...
Visual verification of frontend implementations via Chrome MCP (Claude Chrome Extension) or `ak:chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
...
- **Codebase:** `ak:docs-seeker` skill for package/plugin docs; `repomix` for codebase summary
- **Scouting:** `/ak:scout` or `/ak:scout ext` for finding relevant files
- **Frontend:** Chrome browser or `ak:chrome-devtools` skill for visual verification (screenshots, console, network)
