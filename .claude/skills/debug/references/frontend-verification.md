...
Visual verification of frontend implementations using Chrome MCP (Claude Chrome Extension) or `ak:chrome-devtools` skill fallback.
...
Check if Chrome MCP server is available via `ak:mcp-management` skill or `ListMcpResourcesTool`:
...
When Chrome MCP is not configured, use `ak:chrome-devtools` skill (Puppeteer with bundled Chromium):
...
If `ak:chrome-devtools` skill is also unavailable, skip visual verification and note in report:
