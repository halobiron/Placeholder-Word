...
const { loadConfig } = require('./hooks/lib/ak-config-utils.cjs');
...
async function main() {
  try {
    ...
    try {
      ...
      if (sessionId) {
        const sessionPath = path.join(os.tmpdir(), `ak-session-${sessionId}.json`);
    ...
    if (sessionId && contextSize > 0) {
      try {
        const contextDataPath = path.join(os.tmpdir(), `ak-context-${sessionId}.json`);
    ...
    try {
      const usageCachePath = path.join(os.tmpdir(), 'ak-usage-limits-cache.json');
