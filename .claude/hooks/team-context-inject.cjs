...
try {
  ...
  const os = require('os');
  const { isHookEnabled } = require('./lib/ak-config-utils.cjs');
...
/**
 * Build AK stack context from environment variables
...
function buildCkContext() {
  ...
  if (env.AK_REPORTS_PATH) ctx.push(`Reports: ${env.AK_REPORTS_PATH}`);
  if (env.AK_PLANS_PATH) ctx.push(`Plans: ${env.AK_PLANS_PATH}`);
  if (env.AK_PROJECT_ROOT) ctx.push(`Project: ${env.AK_PROJECT_ROOT}`);
  if (env.AK_NAME_PATTERN) ctx.push(`Naming: ${env.AK_NAME_PATTERN}`);
  if (env.AK_GIT_BRANCH) ctx.push(`Branch: ${env.AK_GIT_BRANCH}`);
  if (env.AK_ACTIVE_PLAN) ctx.push(`Active plan: ${env.AK_ACTIVE_PLAN}`);
