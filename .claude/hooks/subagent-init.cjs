...
async function main() {
  try {
    ...
    const sessionId = payload.session_id || process.env.AK_SESSION_ID || null;
    ...
    // BMAD Artifacts — inject existing artifact paths for context chain
    const artifactsDir = path.join(baseDir, 'plans', 'artifacts');
    const storiesDir = path.join(baseDir, 'plans', 'stories');
    const sprintsDir = path.join(baseDir, 'plans', 'sprints');
    const bmadArtifacts = [];
    try {
      const prdPath = path.join(artifactsDir, 'PRD.md');
      const archPath = path.join(artifactsDir, 'architecture.md');
      const ctxPath = path.join(artifactsDir, 'project-context.md');
      const sprintPath = path.join(sprintsDir, 'sprint-status.yaml');
      if (fs.existsSync(prdPath)) bmadArtifacts.push(`- PRD: ${prdPath}`);
      if (fs.existsSync(archPath)) bmadArtifacts.push(`- Architecture: ${archPath}`);
      if (fs.existsSync(ctxPath)) bmadArtifacts.push(`- Project Context: ${ctxPath}`);
      if (fs.existsSync(sprintPath)) bmadArtifacts.push(`- Sprint Status: ${sprintPath}`);
      // Count story files
      if (fs.existsSync(storiesDir)) {
        const storyFiles = fs.readdirSync(storiesDir).filter(f => f.startsWith('story-') && f.endsWith('.md'));
        if (storyFiles.length > 0) bmadArtifacts.push(`- Stories: ${storyFiles.length} in ${storiesDir}`);
      }
    } catch { /* fail-open */ }

    if (bmadArtifacts.length > 0) {
      lines.push(``);
      lines.push(`## BMAD Artifacts`);
      lines.push(...bmadArtifacts);
      lines.push(`- Rules: agile-gates.md, tdd-rules.md`);
    }
