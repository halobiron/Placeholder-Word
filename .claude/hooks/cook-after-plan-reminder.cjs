...
try {
  ...
  const { isHookEnabled, readSessionState } = require('./lib/ak-config-utils.cjs');
  ...
  try {
    ...
    console.log('Plan completed. Choose your next step:');
    console.log('  Quick Flow: /cook --auto (small tasks, < 3 files)');
    if (planPath) {
      const planMdPath = path.join(planPath, 'plan.md');
      console.log(`  Quick Flow: /clear then /cook ${planMdPath}`);
    }
    console.log('  BMAD Flow:  /sprint plan → /story dev {slug} (full agile cycle)');
