...
const LOCAL_CONFIG_PATH = '.claude/.ak.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.claude', '.ak.json');
...
function getSessionTempPath(sessionId) {
  return path.join(os.tmpdir(), `ak-session-${sessionId}.json`);
}
 ...
 *   2. Global config (~/.claude/.ak.json) - user preferences
 *   3. Local config (./.claude/.ak.json) - project-specific overrides
 *
...
function resolveNamingPattern(planConfig, gitBranch) {
  ...
  if (!validation.valid) {
    ...
    if (process.env.AK_DEBUG) {
      console.error(`[ak-config] Warning: ${validation.error}`);
    }
  }
