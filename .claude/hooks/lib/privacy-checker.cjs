...
/**
 * Load .ak.json config to check if privacy block is disabled
 * @param {string} [configDir] - Directory containing .ak.json (defaults to .claude in cwd)
 * @returns {boolean} true if privacy block should be skipped
 */
function isPrivacyBlockDisabled(configDir) {
  try {
    const configPath = configDir
      ? path.join(configDir, '.ak.json')
      : path.join(process.cwd(), '.claude', '.ak.json');
 ...
 * @param {string} [params.options.configDir] - Directory for .ak.json config
