...
describe('context-builder.cjs', () => {
  ...
  describe('resolveRulesPath()', () => {
    ...
    it('falls back to workflows/ when rules/ does not exist', () => {
      ...
      createTestFile(path.join(tempDir, '.claude/workflows'), 'test-fallback-rules.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('test-fallback-rules.md');
      assert.strictEqual(result, '.claude/workflows/test-fallback-rules.md',
  ...
  describe('CLAUDE.md reference resolution', () => {
    ...
    it('resolves legacy @workflows/ references via fallback', () => {
      ...
      tempDir = createTempDir(['.claude/workflows']);
      createTestFile(path.join(tempDir, '.claude/workflows'), 'test-legacy-rules.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('test-legacy-rules.md');
