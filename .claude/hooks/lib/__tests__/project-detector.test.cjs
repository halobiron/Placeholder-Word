...
// Simple polyfill for Jest's expect() to work with node:test
const expect = (actual) => ({
  toBe: (expected) => assert.strictEqual(actual, expected),
  toBeTruthy: () => assert.ok(actual),
  toContain: (item) => assert.ok(actual.includes(item)),
  toBeLessThan: (value) => assert.ok(actual < value),
  toMatch: (regex) => assert.match(String(actual), regex),
  not: {
    toThrow: () => { try { actual(); } catch (e) { assert.fail(e); } },
    toMatch: (regex) => assert.doesNotMatch(String(actual), regex)
  }
});
