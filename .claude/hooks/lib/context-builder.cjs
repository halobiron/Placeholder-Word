...
const USAGE_CACHE_FILE = path.join(os.tmpdir(), 'ak-usage-limits-cache.json');
...
function buildContextSection(sessionId) {
  ...
  try {
    const contextPath = path.join(os.tmpdir(), `ak-context-${sessionId}.json`);
 ...
 * @param {Object} [params.config] - AK config (auto-loaded if not provided)
