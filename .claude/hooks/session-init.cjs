...
try {
  ...
  const {
    ...
    extractTaskListId,
    isHookEnabled
  } = require('./lib/ak-config-utils.cjs');
...
async function main() {
  try {
    ...
    if (envFile) {
      // Session & plan config
      writeEnv(envFile, 'AK_SESSION_ID', sessionId || '');
      writeEnv(envFile, 'AK_PLAN_NAMING_FORMAT', config.plan.namingFormat);
      writeEnv(envFile, 'AK_PLAN_DATE_FORMAT', config.plan.dateFormat);
      writeEnv(envFile, 'AK_PLAN_ISSUE_PREFIX', config.plan.issuePrefix || '');
      writeEnv(envFile, 'AK_PLAN_REPORTS_DIR', config.plan.reportsDir);

      // NEW: Resolved naming pattern for DRY file naming in agents
      // Example: "251212-1830-GH-88-{slug}" or "251212-1830-{slug}"
      // Agents use: `{agent-type}-$AK_NAME_PATTERN.md` and substitute {slug}
      writeEnv(envFile, 'AK_NAME_PATTERN', namePattern);

      // Plan resolution
      writeEnv(envFile, 'AK_ACTIVE_PLAN', resolved.resolvedBy === 'session' ? resolved.path : '');
      writeEnv(envFile, 'AK_SUGGESTED_PLAN', resolved.resolvedBy === 'branch' ? resolved.path : '');
      ...
      writeEnv(envFile, 'AK_GIT_ROOT', staticEnv.gitRoot || '');
      writeEnv(envFile, 'AK_REPORTS_PATH', path.join(baseDir, reportsPath));
      writeEnv(envFile, 'AK_DOCS_PATH', path.join(baseDir, config.paths.docs));
      writeEnv(envFile, 'AK_PLANS_PATH', path.join(baseDir, config.paths.plans));
      writeEnv(envFile, 'AK_PROJECT_ROOT', process.cwd());

      // Project detection
      writeEnv(envFile, 'AK_PROJECT_TYPE', detections.type || '');
      writeEnv(envFile, 'AK_PACKAGE_MANAGER', detections.pm || '');
      writeEnv(envFile, 'AK_FRAMEWORK', detections.framework || '');

      // NEW: Static environment info (so other hooks don't need to recompute)
      writeEnv(envFile, 'AK_NODE_VERSION', staticEnv.nodeVersion);
      writeEnv(envFile, 'AK_PYTHON_VERSION', staticEnv.pythonVersion || '');
      writeEnv(envFile, 'AK_OS_PLATFORM', staticEnv.osPlatform);
      writeEnv(envFile, 'AK_GIT_URL', staticEnv.gitUrl || '');
      writeEnv(envFile, 'AK_GIT_BRANCH', staticEnv.gitBranch || '');
      writeEnv(envFile, 'AK_USER', staticEnv.user);
      writeEnv(envFile, 'AK_LOCALE', staticEnv.locale);
      writeEnv(envFile, 'AK_TIMEZONE', staticEnv.timezone);
      writeEnv(envFile, 'AK_CLAUDE_SETTINGS_DIR', staticEnv.claudeSettingsDir);

      // Locale config
      if (config.locale?.thinkingLanguage) {
        writeEnv(envFile, 'AK_THINKING_LANGUAGE', config.locale.thinkingLanguage);
      }
      if (config.locale?.responseLanguage) {
        writeEnv(envFile, 'AK_RESPONSE_LANGUAGE', config.locale.responseLanguage);
      }

      // Plan validation config (for /plan validate, /plan --hard, /plan --parallel)
      const validation = config.plan?.validation || {};
      writeEnv(envFile, 'AK_VALIDATION_MODE', validation.mode || 'prompt');
      writeEnv(envFile, 'AK_VALIDATION_MIN_QUESTIONS', validation.minQuestions || 3);
      writeEnv(envFile, 'AK_VALIDATION_MAX_QUESTIONS', validation.maxQuestions || 8);
      writeEnv(envFile, 'AK_VALIDATION_FOCUS_AREAS', (validation.focusAreas || ['assumptions', 'risks', 'tradeoffs', 'architecture']).join(','));

      // Coding level config (for output style selection)
      const codingLevel = config.codingLevel ?? 5;
      writeEnv(envFile, 'AK_CODING_LEVEL', codingLevel);
      writeEnv(envFile, 'AK_CODING_LEVEL_STYLE', getCodingLevelStyleName(codingLevel));
    ...
    if (envFile && teamInfo) {
      writeEnv(envFile, 'AK_AGENT_TEAM', teamInfo.teamName);
      writeEnv(envFile, 'AK_AGENT_TEAM_MEMBERS', teamInfo.memberCount);
    }
