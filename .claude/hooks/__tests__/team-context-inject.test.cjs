 ...
 * - AK stack context building from environment variables
...
describe('team-context-inject.cjs', () => {
  ...
  describe('AK Stack Context Building', () => {

    it('includes AK context when environment variables are set', async () => {
      const tmpDir = path.join(os.tmpdir(), 'team-inject-ak-' + Date.now());
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        ...
        const result = await runHook({
          ...
          env: {
            HOME: tmpDir,
            AK_REPORTS_PATH: '/project/plans/reports',
            AK_PLANS_PATH: '/project/plans',
            AK_PROJECT_ROOT: '/project',
            AK_GIT_BRANCH: 'main'
          }
        });
        ...
        assert.ok(context.includes('AK Context'), 'Should have AK Context section');
    ...
    it('always includes commit convention in AK context', async () => {
      ...
      try {
        ...
        const result = await runHook({
          ...
          env: { HOME: tmpDir, AK_ACTIVE_PLAN: 'test' }
        });
    ...
    it('only includes AK context lines when env vars exist (fail-open)', async () => {
      const tmpDir = path.join(os.tmpdir(), 'team-inject-empty-ak-' + Date.now());
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        createTestTeam(tmpDir, 'team-a');

        // Run with no AK_* env vars
        const result = await runHook({
          agent_id: 'alice@team-a'
        }, {
          env: { HOME: tmpDir }
        ...
        // Should not have empty AK Context section
        // Commit convention is always present, so AK Context should appear
  ...
  describe('Error Handling & Robustness', () => {
    ...
    it('logs to stderr only when AK_DEBUG is enabled', async () => {
