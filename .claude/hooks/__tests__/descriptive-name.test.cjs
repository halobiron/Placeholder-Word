...
describe('descriptive-name.cjs', () => {
  ...
  describe('Hook disable functionality', () => {
    ...
    it('exits silently when disabled via .ak.json', async () => {
      // Create .claude/.ak.json with hook disabled
      const claudeDir = path.join(tempDir, '.claude');
      fs.mkdirSync(claudeDir, { recursive: true });
      fs.writeFileSync(
        path.join(claudeDir, '.ak.json'),
      ...
      fs.copyFileSync(
        path.join(__dirname, '..', 'lib', 'ak-config-utils.cjs'),
        path.join(libDir, 'ak-config-utils.cjs')
      );
