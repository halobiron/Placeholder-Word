 ...
 *
 * The session temp file (/tmp/ak-session-{id}.json) is the source of truth
 * for plan context within a session. Env vars ($AK_ACTIVE_PLAN) are just
 * the initial snapshot from session start.
 */

const path = require('path');
const { writeSessionState, readSessionState } = require('../hooks/lib/ak-config-utils.cjs');
...
if (!sessionId) {
  console.warn('Warning: AK_SESSION_ID not set - session state will not persist');
