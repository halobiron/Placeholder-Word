#!/usr/bin/env python3
"""
Smoke/integration tests for canonical ak-help script.
...
def count_output_markers(output: str) -> int:
    return len([line for line in output.splitlines() if line.startswith("@AK_OUTPUT_TYPE:")])
...
def test(
    ...
    if expect_type:
        marker = f"@AK_OUTPUT_TYPE:{expect_type}"
...
def main():
    print("=" * 60)
    print("ak-help canonical test suite")
    print("=" * 60)
    ...
    test(
        ...
        expect_contains=["agentkit skills", "quick start", "/code-review"],
    )
