...
## AK Stack Conventions

### Report Output
- Save reports to `{AK_REPORTS_PATH}` (injected via hook, fallback: `plans/reports/`)
...
## Agent Mail Protocol (mcp_agent_mail)

When using mcp_agent_mail for async team coordination:

### Agent Identity
- Each agent gets a memorable identity: `{role}-{adjective}-{animal}` (e.g., `dev-fast-rover`, `qa-sharp-eagle`)
- Identity is set at spawn time and persists for the session
- Use identity as sender/recipient in all messages

### Thread Convention
- Thread ID = story slug: `story-{slug}` (e.g., `story-1-1-user-auth`)
- All communications about a story use the same thread
- Sprint-level discussions use thread: `sprint-{n}`

### File Reservations
- Before editing shared files, reserve via agent mail: `reserve_file(path, identity, ttl_seconds)`
- Check existing reservations before starting edits
- Release reservation when done: `release_file(path, identity)`
- If reservation exists by another agent → STOP and SendMessage to that agent

### Inbox Check
- On start: check inbox for pending messages before beginning work
- After completing a task: check inbox before picking next task
- Process messages in order: `shutdown_request` > `urgent` > `message`
